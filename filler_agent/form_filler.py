...
def fill_in_form(keys: List[str], form_path: str, context_dir: str, output_path: Optional[str] = None) -> str:
    """
    ...
    placeholder_pattern = detect_placeholder_patterns(form_text)
    
    ext = os.path.splitext(form_path)[1].lower()
    if ext == '.docx':
        return fill_docx(keys, form_path, context_dir, output_path, placeholder_pattern)
    elif ext == '.pdf':
        return fill_pdf(keys, form_path, context_dir, output_path, placeholder_pattern)
    else:
