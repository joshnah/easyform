...
def fill_docx(keys: List[str], form_path: str, context_dir: str, output_path: Optional[str], placeholder_pattern) -> str:
    """Fill a DOCX form with context data."""
            ...
            if lines[i:i+n] == group:
                for j, loc in enumerate(locations[i:i+n]):
                    if loc[0] == 'para':
                        ...
                        # Apply original font properties to all runs in the paragraph
                        if original_font_info:
                            ...
                            if original_font_name not in font_cache:
                                ...
                                logging.debug(f"Resolved font for DOCX paragraph: {original_font_name} → {font_name}")
                            else:
                                font_name, font_file_path = font_cache[original_font_name]
                            
                            for run in para.runs:
                                if font_name != "helv":  # Only set font if not using fallback
                                    run.font.name = font_name
                                # Preserve other original font properties
                        ...
                        # loc = ('cell', cell, para, original_font_info)
                        ...
                        if original_font_info:
                            ...
                            if original_font_name not in font_cache:
                                font_name, font_file_path = get_available_font(original_font_name, cache_dir)
                                font_cache[original_font_name] = (font_name, font_file_path)
                                logging.debug(f"Resolved font for DOCX table cell: {original_font_name} → {font_name}")
                            else:
                                font_name, font_file_path = font_cache[original_font_name]
    ...
    # Apply checkbox changes to document
