...
def fill_pdf(keys: List[str], form_path: str, context_dir: str, output_path: Optional[str], placeholder_pattern) -> str:
    ...
    try:
        ...
        logging.info(f"No interactive form or fill failed: {e}, falling back to flat PDF fill.")
        # Fall back to flat PDF fill
        return fill_flat_pdf(keys, form_path, context_dir, output_path, placeholder_pattern)


def fill_flat_pdf(keys: List[str], form_path: str, context_dir: str, output_path: Optional[str], placeholder_pattern) -> str:
                ...
                if lines[i:i+n] == group:
                    for j in range(n):
                        ...
                        if original_font_name and original_font_name not in font_cache:
                            ...
                            logging.debug(f"Resolved font for PDF span: {original_font_name} → {font_name}")
                        ...
                        if font_file_path and os.path.exists(font_file_path):
                            try:
                                ...
                                text_inserted = True
                                logging.debug(f"Used custom font file for span: {font_file_path}")
                            except Exception as e:
                                logging.debug(f"Failed to use custom font file {font_file_path}: {e}")
                        ...
                        if not text_inserted:
                            try:
                                ...
                                logging.debug(f"Failed to use system font {font_name}: {e}")
                        
                        # Final fallback to Arial
                        if not text_inserted:
                            try:
                                page.insert_text((x, y), sanitized_text, fontname="Arial", fontsize=fontsize, color=(0,0,0))
                                text_inserted = True
