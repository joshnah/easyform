...
_groq_min_interval = 2.0 if GROQ_FREE_TIER_MODE else 0.2  # 2 seconds for free tier, 0.2 second for paid
...
def query_gpt(prompt: str, model: Optional[str] = None, provider: Optional[Literal["openai", "groq"]] = None) -> str:
    ...
    if provider == "openai":
        ...
        if time_since_last < _groq_min_interval:
            ...
            logging.info(f"Rate limiting: waiting {sleep_time:.1f}s before Groq request")
    ...
    for retry in range(_max_retries):
        try:
            ...
            if provider == "openai" and OPENAI_AVAILABLE:
                if hasattr(_openai_module.error, 'RateLimitError') and isinstance(e, _openai_module.error.RateLimitError):
                    is_rate_limit = True
            elif provider == "groq":
                # Groq rate limiting detection - check for various rate limit indicators
                error_str = str(e).lower()
                if any(indicator in error_str for indicator in ["rate limit", "429", "too many requests", "quota"]):
                    is_rate_limit = True
            
            if is_rate_limit:
                # Exponential backoff with longer delays for Groq
                if provider == "groq":
                    base_wait = 10.0 if GROQ_FREE_TIER_MODE else 5.0  # Longer wait for free tier
                else:
                    base_wait = 2.0
                wait = base_wait * (_backoff_factor ** retry)
                logging.warning(f"Rate limit reached for {provider}, retrying in {wait:.1f} seconds (retry {retry+1}/{_max_retries})")
                time.sleep(wait)
